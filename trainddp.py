...
REDUCE_LR_PATIENCE = 10  # 多少个epoch无改善就降低学习率
...
# 3. 辅助函数：绘制训练曲线
# ==========================================
def plot_original_style(data, save_path):
    """
    绘制原版风格曲线（从plot.py移植）
    
    Args:
        data: 包含训练数据的字典，包含epochs, train_mse_losses, test_mse_losses, train_res_losses, test_res_losses
        save_path: 保存路径
    """
    # 在DDP环境中，只在主进程中执行绘图
    ...
    # ==========================================
    # 子图 1: MSE Loss Curve (Log Scale)
    # ==========================================
    plt.subplot(1, 2, 1)
    
    # 绘制曲线 (Blue/Red, marker size=3)
    ...
    # ==========================================
    # 子图 2: Residual Loss Curve (Log Scale)
    # ==========================================
    plt.subplot(1, 2, 2)
...
def plot_mse_res_loss(train_mse, test_mse, train_res, test_res, save_path="mse_res_loss.svg"):
    """
    绘制包含MSE loss和RES loss的训练曲线

    Args:
        train_mse: 训练集MSE loss列表
        test_mse: 测试集MSE loss列表
        train_res: 训练集RES loss列表
        test_res: 测试集RES loss列表
        save_path: 保存路径
    """
    # 在DDP环境中，只在主进程中执行绘图
    if not is_main_process():
...
def main_worker(rank, world_size):
    ...
    best_epoch = -1
    best_saved_loss = float('inf')  # 用于保存模型判断的最佳loss
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        for data_list in train_loader:
                ...
                # 【新增】梯度裁剪：防止梯度爆炸导致的 Loss 尖峰
                # max_norm 建议设为 1.0，如果依然震荡可设为 0.5
    ...
    if is_main_process:
        ...
        # 生成训练曲线（使用plot.py中的plot_original_style）
        curve_path = os.path.join(OUTPUT_DIR, "style_loss_curve.pdf")
        plot_original_style(training_data, curve_path)
